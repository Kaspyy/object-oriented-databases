...
db.klienci.find({imie: {$nin: ['Adam', 'Marta', 'Beata']}}, {imie: 1, nazwisko: 1}).sort({nazwisko: 1});
...
db.pokoje.find({dostawka: 'T', cenaZaDobe: {$gt: 300}}, {idPokoju: 1, cenaZaDobe: 1}).sort({cenaZaDobe: -1});

// Wyświetl wszystkich klientów którzy mieszkają w domu w Warszawie przy ulicy Polnej.
db.klienci.find({'adresZamieszkania.miejscowosc': 'Warszawa', 'adresZamieszkania.ulica': 'Polna', 'adresZamieszkania.nrMieszkania': {$exists: false}});
